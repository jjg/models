...
module panel(){
    ...
    difference(){
        ...
        translate([BASE_LENGTH*.222,BASE_WIDTH/2,-30]){
            rotate([0,0,90]){
        ...
        translate([BASE_LENGTH*.60+FRAME_THICKNESS,BASE_WIDTH*.53+FRAME_THICKNESS,-1]){
            ...
            translate([subpanel_length/2,subpanel_width/2,-WALL_THICKNESS-1]){
                cylinder(r=LARGE_ILLUMINATED_BUTTON_RADIUS/2,h=(WALL_THICKNESS*2)+2);
            }
            translate([0,0,WALL_THICKNESS]){
...
module meter_frame(){
    ...
    color("white")
    difference(){
        cube([meter_diameter,meter_diameter,WALL_THICKNESS]);
        translate([WALL_THICKNESS,WALL_THICKNESS,-1]){
            cube([meter_diameter-WALL_THICKNESS*2,meter_diameter-WALL_THICKNESS*2,WALL_THICKNESS+2]);
        }
    }
}
...
module preview(explode=0){
    ...
    translate([0,0,(WALL_THICKNESS*2)+explode]){
        ...
        panel();
        // panel meter
        translate([BASE_LENGTH*.222,BASE_WIDTH/2,-30+explode]){
            rotate([0,0,90]){
                hua_5vdc_panel_meter(true);
            }
            // meter frame
            translate([-45,-45,36+explode]){
                meter_frame();
            }
        }
    }        
