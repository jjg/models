...
module panel(){
    ...
    difference(){
        ...
        translate([BASE_LENGTH*.40+FRAME_THICKNESS,BASE_WIDTH*.40+FRAME_THICKNESS,-1]){
            subpanel_length = LARGE_PANEL_LENGTH-(FRAME_THICKNESS*2);
            subpanel_width = LARGE_PANEL_WIDTH-(FRAME_THICKNESS*2);
            
            translate([subpanel_length/4,subpanel_width/4,-WALL_THICKNESS-1]){
                cylinder(r=SMALL_TOGGLE_RADIUS/2,h=(WALL_THICKNESS*2)+2);
            }
            translate([subpanel_length-(subpanel_length/4),subpanel_width/4,-WALL_THICKNESS-1]){
                cylinder(r=SMALL_TOGGLE_RADIUS/2,h=(WALL_THICKNESS*2)+2);
            }
            translate([subpanel_length/4,subpanel_width-(subpanel_width/4),-WALL_THICKNESS-1]){
                cylinder(r=SMALL_TOGGLE_RADIUS/2,h=(WALL_THICKNESS*2)+2);
            }
            translate([subpanel_length-(subpanel_length/4),subpanel_width-(subpanel_width/4),-WALL_THICKNESS-1]){
                cylinder(r=SMALL_TOGGLE_RADIUS/2,h=(WALL_THICKNESS*2)+2);
            }
        ...
        translate([BASE_LENGTH*.60+FRAME_THICKNESS,BASE_WIDTH*.53+FRAME_THICKNESS,-1]){
            ...
            translate([subpanel_length/2,subpanel_width/2,-WALL_THICKNESS-1]){
                cylinder(r=POT_RADIUS/2,h=(WALL_THICKNESS*2)+2);
            }
            translate([0,0,WALL_THICKNESS]){
        ...
        translate([BASE_LENGTH*.80,BASE_WIDTH*.42,-1]){
            cylinder(r=LARGE_LED_RADIUS/2,h=(WALL_THICKNESS*2)+2);
        }
...
module preview(explode=0){
    ...
    translate([0,0,(WALL_THICKNESS*2)+explode]){
        ...
        panel();
        // panel meter
        translate([BASE_LENGTH*.10,(BASE_WIDTH/2)-(SCREEN_WIDTH/2),explode]){
            screen_frame();
        }
    }        
}